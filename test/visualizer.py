...
# Force NumRects & Instance to be real Python ints (fixes type checker warnings)
df['NumRects'] = df['NumRects'].astype(int)
...
# Fix NaN std values (happens with 1 sample groups)
stats = stats.fillna(0)

# Convert all numpy scalars → Python types (eliminates "Scalar → ConvertibleToInt" errors)
stats = stats.astype({
    'NumRects': int,
    'mean_boxes': float,
    'std_boxes': float,
...
groups = [f"{int(r)} rects" for r in stats['NumRects']]
x = np.arange(len(groups), dtype=int)
...
ax.set_ylim(0, float(stats['max_boxes'].max()) * 1.2)
...
ax.plot(stats['NumRects'], stats['mean_cpu'],  marker='o', color=BLUE,
        linewidth=2, label='CPU Time (ms)')
ax.plot(stats['NumRects'], stats['mean_wall'], marker='s', color=ORANGE,
        linewidth=2, linestyle='--', label='Wall Time (ms)')
...
for nr, grp in df.groupby(df['NumRects'].astype(int)):
    color = colors_map.get(nr, '#777777')

    ax.scatter(grp['Instance'], grp['NumBoxes'], label=f'{nr} rects',
               color=color, s=60, zorder=3)
    ax.plot(grp['Instance'], grp['NumBoxes'], color=color,
...
ax.plot(stats['NumRects'], stats['mean_boxes'], marker='o', color=BLUE,
        linewidth=2, label='Mean Boxes')
